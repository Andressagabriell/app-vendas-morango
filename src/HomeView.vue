...
// Estado para armazenar os dados brutos
const vendas = ref([])

// --- LÓGICA DE CÁLCULO DO DASHBOARD ---

// Propriedade computada para o gráfico de vendas da semana
const dadosGraficoVendas = computed(() => {
  const labels = ['Dom', 'Seg', 'Ter', 'Qua', 'Qui', 'Sex', 'Sáb'];
  const data = Array(7).fill(0); // [0, 0, 0, 0, 0, 0, 0]
  ...
  for (const venda of vendas.value) {
    ...
    // Verifica se a venda ocorreu na semana atual
    if (dataVenda >= primeiroDiaSemana) {
      const diaDaSemana = dataVenda.getDay(); // 0 = Domingo, 1 = Segunda, ...
...
// Propriedade computada para o monitoramento de clientes
...
.aumento { color: #4caf50; } /* Verde */
.diminuicao { color: #f44336; } /* Vermelho */
.estavel { color: #9e9e9e; } /* Cinza */
</style>
