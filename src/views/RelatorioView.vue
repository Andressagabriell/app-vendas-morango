...
async function buscarVendas() {
  const { data, error } = await supabase
    .from('vendas')
    .select('id, quantidade_caixas, clientes(id, nome), produtos_v2(id, nome), entregue')
...
async function alternarStatusEntrega(venda) {
  ...
  if (error) console.error(error)
    ...
    if (index !== -1) vendas.value[index].entregue = novoStatus
  }
}
      ...
      <div v-for="cliente in relatorioAgrupado" :key="cliente.nomeCliente" class="cliente-card">
        ...
        <ul>
          <li v-for="venda in cliente.produtos" :key="venda.id" :class="{ 'entregue': venda.entregue }">
            <input type="checkbox" :checked="venda.entregue" @change="alternarStatusEntrega(venda)" />
...
h2 { margin-bottom: 1rem; border-bottom: 2px solid #17a2b8; padding-bottom: 0.5rem; }
