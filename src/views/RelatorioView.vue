...
async function buscarVendas() {
  ...
  const { data } = await supabase
    .from('vendas')
    .select(`*, clientes:clientes_v2(nome), produtos_v2(nome)`)
...
const relatorioAgrupado = computed(() => {
  ...
  for (const venda of vendas.value) {
    ...
    const clienteId = venda.clientes.id; // Isso pode não existir mais, vamos usar o nome
    if (!agrupado[venda.clientes.nome]) {
      agrupado[venda.clientes.nome] = {
        nomeCliente: venda.clientes.nome,
        produtos: []
      }
    }
    agrupado[venda.clientes.nome].produtos.push(venda)
  }
...
.relatorio-container { display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 1.5rem; }
.cliente-card { padding: 1.5rem; border: 1px solid #eee; border-radius: 8px; background-color: #fff; }
h2 { margin-bottom: 1rem; border-bottom: 2px solid hsla(160, 100%, 37%, 1); padding-bottom: 0.5rem; }
ul { list-style: none; padding: 0; }
li { display: flex; align-items: center; gap: 1rem; padding: 0.75rem; border-radius: 4px; margin-bottom: 0.5rem; background-color: #f9f9f9; transition: background-color 0.3s; }
